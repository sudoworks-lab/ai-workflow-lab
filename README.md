...
記録した内容に対して、形式チェック、公開状態を維持するための安全性チェック、作業結果の評価、前回結果との差分確認、レビュー要否の判定、レポート生成を行います。
...
このコマンドでは、形式チェック、公開状態を維持するための安全性チェック、作業結果の確認、前回結果との差分確認、レポート生成までをまとめて実行します。
...
## Repository Map
    ...
    ├── LICENSE                      MIT License
    └── TODO_public_review.md        公開レビューと継続確認のチェックリスト
...
公開状態を維持するためには次を確認します。

    npm run cli -- run-all
    rg -n "token|secret|password|api_key|credential|client_secret|private|customer|顧客|社内|実名|email|mail" .

自動チェックの結果だけで安全とみなさず、更新時には `reports/latest.md`、追跡対象の `artifacts/latest/`、`TODO_public_review.md` を目視確認します。
